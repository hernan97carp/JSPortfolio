describe('Navbar Test', () => {
	...
	it('TC2:Validate the text of Presentation', () => {
		...
		//personalInformation
		cy.get('h3.font-Chakra.font-medium.text-2xl.md\\:text-3xl.mt-0.text-center.subtitleBorder.uppercase').contains('Sobre mi').should('exist');

		cy.fixture('home/home.Text').then(the => {
			const textRegards = the.presentation.personalInformation;

			cy.get('p.font-Chakra.font-medium.text-base.text-center.md\\:text-left.w-full.mt-3.p_Mod')
				.invoke('text')
				.then(text => {
	...
	it('TC4:Validate the existence of the all skills', () => {
		cy.get('h3.font-Chakra.font-medium.text-2xl.md\\:text-3xl.mt-0.text-center.subtitleBorder.uppercase').contains('Skills').should('exist');
